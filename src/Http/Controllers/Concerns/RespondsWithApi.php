...
namespace WeiJuKeJi\LaravelDictionary\Http\Controllers\Concerns;
...
trait RespondsWithApi
{
    ...
    protected function getApiResponseClass(): string
    {
        ...
        return \WeiJuKeJi\LaravelDictionary\Support\ApiResponse::class;
    }
     ...
     * 统一解析分页参数，便于在控制器间复用。
     */
    protected function resolvePerPage(array &$filters, ?int $default = null, ?int $max = null): int
    {
        $default = $default ?? config('dictionary.per_page', 15);
        $max = $max ?? config('dictionary.max_per_page', 100);
